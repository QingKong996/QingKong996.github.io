---
title: 'XiDianCTF MoeCTF 2025 PWN 1 Ez_u64'
...
[Challenge link](https://ctf.xidian.edu.cn/training/22?challenge=872)
...
Notice that `num` is a random number.
...
`write` outputs the raw in-memory representation of `num` directly.

`scanf("%zu")` reads an unsigned integer of type `size_t`.

The pwntools script is as follows:
...
> pwntools packers infer `endian` and `signed` from the current context.[^packer]
>
> `unpack` converts a binary byte stream into an integer.
>
> `pack` converts an integer into a binary byte stream.
